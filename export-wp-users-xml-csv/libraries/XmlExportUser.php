...
if (!class_exists('XmlExportUser')) {
    ...
    final class XmlExportUser
    {
        ...
        public static function prepare_data($user, $exportOptions, $xmlWriter = false, &$acfs, $implode_delimiter, $preview)
        {
            ...
            $processCustomFieldsFactory = new \Pmue\Common\UserExport\ProcessCustomFieldsFactory();
            $processCustomFields = $processCustomFieldsFactory->create();
                    ...
                    if (isset($exportOptions['cc_combine_multiple_fields'][$ID]) && $exportOptions['cc_combine_multiple_fields'][$ID]) {
                        ...
                        switch ($fieldType) {
                            ...
                            case 'user_registered':
                                ...
                                break;
                            case 'cf':
                                $article = $processCustomFields->processCustomFields($user->ID, $implode_delimiter, $fieldLabel, $fieldValue, $fieldSnipped, $article, $element_name);
